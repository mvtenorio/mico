			...
			<article class="col-xs-6 col-md-4 item">
				<div class="thumbnail">
					<a href="{{ route('items.edit', $item->id) }}" class="darken">
						<img width="200" src="{{ $item->isObject() ? 'http://www.publicdomainpictures.net/pictures/40000/nahled/question-mark.jpg' : 'http://www.graphicsfuel.com/wp-content/uploads/2012/01/cardboard-box-icon-512x512.png' }}" alt="{{ $item->name }}">
					</a>
					<div class="options">
						<h4 class="text-center">{{ str_limit($item->name, $limit = 15, $end = '...') }}</h4>
